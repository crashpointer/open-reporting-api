...
public class TransactionRepositoryTest extends AbstractDAOTest {
    ...
    @Test
    public void findByTransactionUUID() {
        Optional<Transaction> transaction = transactionRepository.findByTransactionUUID(transactionUUID);

        assertTrue(transaction.isPresent());
        assertTrue(transaction.get().getId() > 0);
        assertEquals(referenceNo, transaction.get().getReferenceNo());
    }

    @Test
    public void findByReferenceNo() {
        Optional<Transaction> transaction = transactionRepository.findByReferenceNo(referenceNo);

        assertTrue(transaction.isPresent());
        assertTrue(transaction.get().getId() > 0);
        assertEquals(transactionUUID, transaction.get().getTransactionUUID());
    }

    @Test
    public void existsByTransactionUUID() {
        assertTrue(transactionRepository.existsByTransactionUUID(transactionUUID));
    }

    @Test
    public void existsByReferenceNo() {
        assertTrue(transactionRepository.existsByReferenceNo(referenceNo));
    }
